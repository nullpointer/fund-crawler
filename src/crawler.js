'use strict';
var path = require('path'), util = require('util'), EventEmitter = require('events').EventEmitter, request = require('request'), _ = require('lodash'), cheerio = require('cheerio'), fs = require('fs'), charsetParser = require('charset-parser'), Bottleneck = require('bottleneckp'), seenreq = require('seenreq'), iconvLite = require('iconv-lite'), typeis = require('type-is').is;
...
function contentType(res) {
    return get(res, 'content-type').split(';').filter(function (item) { return item.trim().length !== 0; }).join(';');
}
...
function Crawler(options) {
    var self = this;
    options = options || {};
    if (["onDrain", "cache"].some(function (key) { return key in options; })) {
    ...
    if (options.jQuery === 'whacko') {
        ...
        readJqueryUrl(scriptLocation, function (err, jquery) {
            try {
                ...
                options.callback(e, { options: options }, options.release);
            }
        });
    }
...
Crawler.prototype.direct = function direct(options) {
    ...
    self.globalOnlyOptions.forEach(function (globalOnlyOption) { return delete options[globalOnlyOption]; });
...
Crawler.prototype.queue = function queue(options) {
    ...
    for (var i = 0; i < options.length; ++i) {
        ...
        self._pushToQueue(_.isString(options[i]) ? { uri: options[i] } : options[i]);
    }
};
